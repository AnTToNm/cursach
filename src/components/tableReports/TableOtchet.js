...
const OotchetTable = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:3001/cattle-otchet', {
                    params: {
                        page: page,
                        limit: 20,
                        column: sortColumn,
    ...
    return (
        <TableWrapper>
            <h2 style={{ display: 'flex', justifyContent: 'center' }}>Отчетная таблица</h2>
            <Table>
                <thead>
                <TR>
                    <TH onClick={() => handleSort('animal')}>
                        Animal{getSortIcon('animal')}
                    </TH>
